			...
			<div class="col-md-8 collapsible-container">
				...
				<ul class="collapsible popout" data-collapsible="accordion">
					...
					@foreach($tickets as $ticket)
						...
						<?php $color = lighten('00bcd4', (1 - ($i/count($tickets))));//getClosest(hexdec(dechex(floor((abs(sin(hexdec(substr($ticket->title, 0, strlen($ticket->title)/2.5)))* 16777215)) % 16777215))) , $colors);?>
						<li class="card">
							<div class="card-title collapsible-header @if(config('app.debug')) themer--secondary @endif" style="background:#{{  $color }}">
							...
							<div class="collapsible-body @if(config('app.debug')) themer--accent-1 @endif" style="background-color:#{{ lighten($color, 0.3)}} !important;">
								<p>
									{{$ticket->body}}
								</p>
								...
								@if($ticket->comments->count() > 0)
									...
									<li class="card " style="border-radius:5px; ">
										...
										<div class="collapsible-body @if(config('app.debug')) themer--accent-2 @endif"><p>{{$comment->body}}</p></div>
									</li>
